...
def get_schedule():
    ...
    try:
        ...
        print(f"Received filters - Schools: {school_ids}, Groups: {group_ids}, Teachers: {teacher_ids}, Years: {years}")
        ...
        formatted_schedule = format_schedule_data_unified(raw_schedule, school_ids, group_ids, teacher_ids, years)
...
def format_schedule_data_unified(raw_data, school_ids=[], group_ids=[], teacher_ids=[], years=[]):
    ...
    if school_ids:
        if len(school_ids) == 1:
            school_name = next((item['school_name'] for item in raw_data if item['school_id'] == school_ids[0]), f"Szkoła {school_ids[0]}")
    ...
    if group_ids:
        if len(group_ids) == 1:
            group_name = next((item['group_name'] for item in raw_data if item['group_id'] == group_ids[0]), f"Klasa {group_ids[0]}")
    ...
    if teacher_ids:
        if len(teacher_ids) == 1:
            teacher_name = next((item['teacher_name'] for item in raw_data if item['teacher_id'] == teacher_ids[0]), f"Nauczyciel {teacher_ids[0]}")
    ...
    if not any([school_ids, group_ids, teacher_ids, years]):
        ...
        for year, year_items in years_data.items():
            ...
            schedule[year_key] = create_unified_schedule(year_items, f"Wszystkie klasy - Rok {year}", "Wszystkie szkoły", times, days)
    else:
        # Jeden wspólny plan dla wszystkich wybranych filtrów
        main_title = " - ".join(title_parts)
        schedule[main_title] = create_unified_schedule(raw_data, main_title, title_parts[0] if title_parts else "Plan zajęć", times, days)
